...
import fetch, { Headers, Request, RequestInit, Response } from "isomorphic-fetch-ponyfill";
...
export class Queue {
  ...
  async #rateLimitThrottle() {
    while (this.manager.isGlobalLimited() || this.isLocalLimited()) {
      ...
      if (isGlobal) {
        log(`Global rate limit reached. Blocking all requests for ${timeout}ms`);
      } else {
        log(`${this.id} encountered a local rate limit. Waiting ${timeout}ms before continuing`);
      }
